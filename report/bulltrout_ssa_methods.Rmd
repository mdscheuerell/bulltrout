...
where $y_{i,t}$ is the log-count for population $i$ and year $t$, $a_i$ is an offset to account for partial sampling in population $i$, and $v_{i,t} \sim \text{N}(0, r_i)$\footnote{Here the variance of the observation errors is assumed to be population specific, but it might be reasonable to assume that they all have the same variance, such that $v_{i,t} \sim \text{N}(0, r)$.}. We can combine each of the population specific observation models into a matrix form, such that
...
where $u_i$ is the unique bias for each population, and $w_{i,t} \sim \text{N}(0, q_i)$\footnote{Here the variance of the process errors is assumed to be population specific, but it might be reasonable to assume that they all have the same variance, given their membership within a core area, such that $w_{i,t} \sim \text{N}(0, q)$.}.
...
At this point, however, we are assuming that the monitoring data for each population is telling us something about only itself, rather than contributing information to the population trend at the larger scale of their core area.
