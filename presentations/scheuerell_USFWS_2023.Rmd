...
class: frimg
background-image: url(figs/wacfwru.png)
background-size: 95%
...
class: frimg
background-image: url(figs/mark_safs.png)
background-size: 100%
...
.blue-text[
### Required every 5 years to ensure appropriate protections
...
* ### .green-text[population trends].gray-text[, distribution, demographics & genetics]
...
# Ex: biased random walk with obs error
...
plot.ts(x1, las = 1, col = "gray", lwd = 3, ylim = c(9, 15), # range(x1, y1),
        ylab = expression(italic(x[t])), main = "")
mtext("Positive bias", side = 3, line = 1, cex = 2)
lines(y1, col = "dodgerblue", lwd = 3)

plot.ts(x2, las = 1, col = "gray", lwd = 3, ylim = c(6, 11), # range(x2, y2),
        ylab = expression(italic(x[t])), main = "")
mtext("Negative bias", side = 3, line = 1, cex = 2)
...
.blue-text[ 
## Different methods have different observation errors

## Ex: Weir counts are more precise than snorkel surveys
]
...
## .center.blue-text[ Example of 5 popns, 2 core areas via 2 methods (i & j)]

$$\begin{gathered}
    v_{1,t} \sim \text{N}(0, r_i) \\
    v_{2,t} \sim \text{N}(0, r_j) \\
    \hline 
    v_{3,t} \sim \text{N}(0, r_j) \\
    v_{4,t} \sim \text{N}(0, r_i) \\
...
## .blue-text[ Used the 30-year time period from 1991-2020 ]

## .blue-text[ Time series needed &ge;10 years of non-missing data ]
...
## .green-text[ All models fit in **R** using the `{MARSS}` package ]

## .green-text[ Estimated 90% confidence intervals on bias terms ]
